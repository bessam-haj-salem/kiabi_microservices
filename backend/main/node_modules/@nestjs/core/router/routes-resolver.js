            ...
            routerPaths.forEach(path => {
                ...
                if (!controllerVersion) {
                    pathsToLog.forEach(path => {
                        const logMessage = messages_1.CONTROLLER_MAPPING_MESSAGE(controllerName, path);
                        this.logger.log(logMessage);
                    });
                }
                else {
                    pathsToLog.forEach(path => {
                        const logMessage = messages_1.VERSIONED_CONTROLLER_MAPPING_MESSAGE(controllerName, path, controllerVersion);
                        this.logger.log(logMessage);
                    });
                }
